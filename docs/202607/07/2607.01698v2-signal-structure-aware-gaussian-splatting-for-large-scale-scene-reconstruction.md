...
tldr: 大规模场景中稀疏观测区域导致高斯初始化点不足，高频监督引发无序密集和冗余。提出信号结构感知的SIG调度器，通过场景频率收敛调节图像分辨率和致密化过程，并引入球约束高斯利用点云先验。实现了频率一致、几何感知、无漂浮物训练，在大规模场景中效率和质量均显著优于现有方法。该工作从信号结构恢复角度重新定义了场景重建问题。
source: arxiv
...
motivation: 大规模场景稀疏观测导致高斯无序密集和冗余，现有硬编码调度策略无法感知场景频率收敛行为。
method: 提出SIG调度器，基于场景频率收敛调节训练图像分辨率和高斯致密化；引入球约束高斯利用初始化点云空间先验。
result: 在大规模场景中实现频率一致、几何感知、无漂浮物训练，渲染质量和效率均达到最先进水平。
conclusion: 信号结构感知框架有效解决了大规模场景3D高斯泼溅的过度密集问题，显著提升性能。
---

## 摘要
3D高斯溅射在新视角合成中展现出显著潜力。与小规模场景相比，大规模场景不可避免地包含稀疏观测区域，初始点过于稀疏。在这种情况下，用高频图像监督从低频稀疏点初始化的高斯分布，常常会引发不受控制的致密化和冗余图元，降低效率和质量。直观上，这一问题可以通过调度策略缓解，这些策略可分为两种范式：通过致密化调制目标信号频率，以及通过图像分辨率调制采样频率。然而，先前的调度策略主要是硬编码的，无法感知场景频率的收敛行为。为此，我们从信号结构恢复的角度重新定义场景重建问题，并提出了SIG，一种新的调度器，可使图像监督与高斯频率同步。具体来说，我们推导了3D表示的平均采样频率和带宽，然后基于场景频率收敛调节训练图像分辨率和高斯致密化过程。此外，我们引入了球约束高斯，利用初始化点云的空间先验来控制高斯优化。我们的框架实现了频率一致、几何感知且无浮动点的训练，在大规模场景中，效率和渲染质量均大幅领先现有方法，达到最先进的性能。代码可在 https://github.com/weiyixue999/Signal_Structure_Aware_Gaussian 获取。
